...
void printTossing()
{
	UI::printDelayWithText(2, "Tossing the deck");
}

void printMonsterSelection()
{
	UI::printDelayWithText(2, "Selecting monster card");
}
...
void printMunchkinLost(Monster* monster)
{
	...
	// Your Level decreased by 1. Your level: \ " << Munchkin->getLevel()"
}
void printLevelDecreased(Munchkin* munchkin)
{
	std::cout << "------Your Level decreased. Your level: " << munchkin->getLevel() << "\n";
}
		...
		while (!fight.getFinish())
		{
			...
			else
			{
				...
				if (input == UI::UserInput::Runaway)
				{
					...
					UI::printLevelDecreased(&m_munchkin);
					UI::pressAnyKeyToContinue();
				}
...
void Game::generateMunchkinInitialCards()
{
	...
	const int numberOfModifiers = 5;
	m_itemsDeck.loadDatabaseFromFile("items.txt");


	std::vector<Item*> initialItems = m_itemsDeck.generateItems(numberOfItems);

	std::vector<Modifier*> initialModifiers = m_modifiersDeck.generateModifiers(numberOfModifiers);

	m_munchkin.setItems(initialItems);
	m_munchkin.addModifiers(initialModifiers);
}
...
void Game::generateVictoryBonus()
{
	std::vector<Modifier*> victoryBonus = m_modifiersDeck.generateModifiers(1);

	const std::vector<Modifier*>& currentModifiers = m_munchkin.getModifiers();

	std::vector<Modifier*> newModifiers(currentModifiers);
	newModifiers.insert(newModifiers.end(), victoryBonus.begin(), victoryBonus.end());

	m_munchkin.addModifiers(newModifiers);

	Item* victoryItem = m_itemsDeck.generateItems(1).front();
	m_munchkin.addItem(victoryItem);
}